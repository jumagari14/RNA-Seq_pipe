  ...
  test<- toGRanges(gff_data[gff_data[,"Chromosome"]==length_chr[i,1],c("Chromosome","Start","End","Class")])
  ...
  kpPlotRainfall(kp, data = test,col = test$Class)
  ...
  dev.off()
  # ANalysis of SRR
  test<- toGRanges(gff_data[gff_data[,"Chromosome"]==length_chr[i,1] & gff_data[,"Code"]!="SRR",c("Chromosome","Start","End","Class")])
  custom.genome<- toGRanges(data.frame(Chromosome=length_chr[i,1],start=1,end=length_chr[i,2]))

  png(paste0("Rainfall_not_SRR",length_chr[i,1],".png"),1000,1000)
  kp <- plotKaryotype(plot.type=1,genome = custom.genome)

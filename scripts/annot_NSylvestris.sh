...
module load python/3.7.2 ## Load corresponding Python 3 module available
...
awk '/^>/{if (l!="") print l; print; l=0; next}{l+=length($0)}END{print l}' "$fasta" > lengths.txt
python3 scripts/consensus_gff.py -l lengths.txt -o consensus.gff
...
mkdir -p -m 755 ./bin/STAR_Align_strict  
mkdir -p -m 755 ./bin/counts_strict
...
parallel --compress -j 3 "./scripts/STAR --runThreadN 16 --runMode alignReads --genomeLoad LoadAndKeep --outFilterMismatchNoverReadLmax 0.15 --readFilesCommand zcat --outFilterMatchNmin 16 --outSAMtype BAM Unsorted SortedByCoordinate --limitBAMsortRAM 10000200000 --genomeDir ./bin/genome_ind  --outFileNamePrefix ./bin/STAR_Align_strict/{} --readFilesIn ./bin/trimm_data/{}_1.par.fq.gz  ./bin/trimm_data/{}_2.par.fq.gz" ::: $list
...
cd ./bin/STAR_Align_strict
parallel -j 3 "~/scripts/featureCounts -p -t transcript -g gene_id -F GTF -Q 32 -T 16 -a ~/consensus.gff -o ../counts_strict/{.}.txt {}" ::: *Aligned.out.bam 
...
# -o ../counts_strict/{.}.txt: output prefix 


cd ../counts_strict ; touch Matrix_data_strict.tabular 
...
do 
    ...
    then 
        ...
        paste Matrix_data_strict.tabular count_ind > temp2 && mv temp2 Matrix_data_strict.tabular 
        rm -f temp2
    else
        sed '1d' "$J"Aligned.out.txt > tmp ; mv tmp "$J"Aligned.out.txt 
...
cd ../; rm -f counts_strict/count_ind 

mv counts/Matrix_data_strict.tabular ./
